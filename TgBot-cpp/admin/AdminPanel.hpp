...
class AdminPanel {
public:
    static constexpr long long ADMIN_ID = 000000000LL; // вставь свой tg_id
...
private:
    ...
    void sendAdminMenu(long long cid) {
        ...
        if (r.status_code == 200) {
            try {
                ...
                ss << "⚠ Статистика недоступна\n\n";
            }
        } else {
            // Если эндпоинта ещё нет — показываем меню без статистики
            ss << "⚠ <i>Эндпоинт /admin/stats не добавлен в бэкенд</i>\n\n";
        }
    ...
    void showUsers(int page) {
        ...
        if (r.status_code != 200) {
            m_bot.getApi().sendMessage(ADMIN_ID,
                "⚠ Эндпоинт <code>/admin/users</code> не добавлен в бэкенд.\n\n"
        ...
        try {
            ...
            if (page > 0) {
                ...
                bPrev->text = "◀"; bPrev->callbackData = "adm_users_" + std::to_string(page-1);
                navRow.push_back(bPrev);
            }
            if (page < pages-1) {
                auto bNext = std::make_shared<TgBot::InlineKeyboardButton>();
                bNext->text = "▶"; bNext->callbackData = "adm_users_" + std::to_string(page+1);
            ...
            bBack->text = "◀ Меню"; bBack->callbackData = "adm_refresh";
    ...
    void showAgents(int page) {
        ...
        if (r.status_code != 200) {
            m_bot.getApi().sendMessage(ADMIN_ID,
                "⚠ Эндпоинт <code>/admin/agents</code> не добавлен в бэкенд.",
                nullptr, nullptr, nullptr, "HTML");
            return;
        }
        ...
        try {
            ...
            if (page > 0) {
                auto bPrev = std::make_shared<TgBot::InlineKeyboardButton>();
                bPrev->text = "◀"; bPrev->callbackData = "adm_agents_" + std::to_string(page-1);
                navRow.push_back(bPrev);
            }
            if (page < pages-1) {
                auto bNext = std::make_shared<TgBot::InlineKeyboardButton>();
                bNext->text = "▶"; bNext->callbackData = "adm_agents_" + std::to_string(page+1);
                navRow.push_back(bNext);
            }
            if (!navRow.empty()) kb->inlineKeyboard.push_back(navRow);
    ...
    void giveRank(const std::string& tgid, const std::string& rank, const std::string& days) {
        ...
        if (r.status_code == 200) {
            ...
            try {
                ...
                std::string msg;
                if (rank == "FREE") {
                    msg = "ℹ Ваша подписка была изменена администратором.";
                } else {
    ...
    void doBroadcast(const std::string& text) {
        ...
        if (r.status_code != 200) {
            ...
            m_bot.getApi().sendMessage(ADMIN_ID,
                "⚠ Эндпоинт <code>/admin/all_user_ids</code> не добавлен в бэкенд.\n\n"
